...
class NordpoolDataCoordinator:
    def __init__(
        ...
        self._data_for_current_hass_date: list | None = (
            None  # Raw price list for current HASS date
        )
        self._date_of_current_data: date | None = (
            None  # The HASS date for which _data_for_current_hass_date is valid
        )

        self._data_for_next_hass_date: list | None = (
            None  # Raw price list for current HASS date + 1
        )
        self._date_of_next_data: date | None = (
            None  # The HASS date for which _data_for_next_hass_date is valid
        )
    ...
    async def _execute_nordpool_call_logic(
        ...
        try:
            ...
            _LOGGER.debug(
                f"Nordpool service call for {date_to_fetch_str} returned: {service_response}"
            )
            ...
            if service_response and isinstance(service_response, dict):
                if len(service_response) == 1:
                    ...
                    _LOGGER.info(
                        f"Extracted area '{area_id}' and price data list from service response."
                    )
                    ...
                    if not determined_currency:
                        currency_entity_id = (
                            f"sensor.nord_pool_{area_id.lower()}_currency"
                        )
                        currency_state_obj = self.hass.states.get(currency_entity_id)
                        if currency_state_obj and currency_state_obj.state not in (
                            None,
                            STATE_UNKNOWN,
                            STATE_UNAVAILABLE,
                        ):
                            ...
                            _LOGGER.info(
                                f"Fetched currency '{determined_currency}' from entity '{currency_entity_id}'."
                            )
                        else:
                            _LOGGER.warning(
                ...
                _LOGGER.warning(
                    f"Nordpool call for {date_to_fetch_str} successful but returned no data or unexpected response: {service_response}"
                )
                return "SUCCESS_NO_DATA", None
        except ServiceValidationError as e:
            if (
                "entry_not_loaded" in str(e).lower()
                or "not loaded" in str(e).lower()
                or "did not set up" in str(e).lower()
            ):
                _LOGGER.warning(
            ...
            _LOGGER.error(
                f"Unexpected error calling Nordpool service for {date_to_fetch_str}: {e}",
                exc_info=True,
            )
            return "ERROR_OTHER", None
    ...
    async def _send_updated_data_to_sensor(self, current_hass_date: date) -> None:
        ...
        if (
            self._data_for_current_hass_date
            and self._date_of_current_data == current_hass_date
        ):
            combined_raw_data.extend(self._data_for_current_hass_date)
            _LOGGER.debug(
                f"Including data for {self._date_of_current_data} (current day) in payload. Points: {len(self._data_for_current_hass_date)}"
            )
        elif self._data_for_current_hass_date:
            _LOGGER.warning(
                f"Not including stale current_day_data (for {self._date_of_current_data}) in payload for HASS date {current_hass_date}."
            )

        expected_next_day_date = current_hass_date + timedelta(days=1)
        if (
            self._data_for_next_hass_date
            and self._date_of_next_data == expected_next_day_date
        ):
            combined_raw_data.extend(self._data_for_next_hass_date)
            _LOGGER.debug(
                f"Including data for {self._date_of_next_data} (next day) in payload. Points: {len(self._data_for_next_hass_date)}"
            )
        elif self._data_for_next_hass_date:
            _LOGGER.warning(
                f"Not including stale next_day_data (for {self._date_of_next_data}) in payload for HASS date {current_hass_date} (expected next: {expected_next_day_date})."
            )

        if combined_raw_data:
            if not self._currency:
                _LOGGER.debug(
                    "Sending data to sensor without currency (will use configured currency if available)."
                )

            payload_to_send = {"currency": self._currency, "raw": combined_raw_data}
            _LOGGER.info(
                f"Sending updated combined data to sensor. Currency: {self._currency}, Total points: {len(combined_raw_data)}"
            )
            await self.data_update_callback(payload_to_send)
            data_sent = True
        else:
            _LOGGER.warning(
                f"No combined_raw_data available to send. Current: {len(self._data_for_current_hass_date) if self._data_for_current_hass_date else 0} items for {self._date_of_current_data}, Next: {len(self._data_for_next_hass_date) if self._data_for_next_hass_date else 0} items for {self._date_of_next_data}"
            )

        if not data_sent and (
            self._data_for_current_hass_date or self._data_for_next_hass_date
        ):
            _LOGGER.debug(
                f"Data was available (current: {self._date_of_current_data}, next: {self._date_of_next_data}) but not sent for HASS date {current_hass_date}, likely due to date mismatch."
            )
    ...
    async def _trigger_and_reschedule_nordpool(
        ...
        if (
            not self._data_for_current_hass_date
            or self._date_of_current_data != current_hass_date
        ):
            target_fetch_date = current_hass_date
            current_operation_type = "TODAY"
            fetch_reason = (
                f"current data missing or stale (is {self._date_of_current_data})"
            )
        elif self._date_of_current_data == current_hass_date and (
            not self._data_for_next_hass_date
            or self._date_of_next_data != (current_hass_date + timedelta(days=1))
        ):
            ...
            _LOGGER.debug(
                f"Data for today ({self._date_of_current_data}) and tomorrow ({self._date_of_next_data}) appears up-to-date for HASS date {current_hass_date}."
            )

        call_status = "NOT_ATTEMPTED"
        if target_fetch_date:
            _LOGGER.info(
                f"Attempting to fetch data for {target_fetch_date} (Operation: {current_operation_type}, Reason: {fetch_reason})"
            )
            call_status, nordpool_day_payload = await self._execute_nordpool_call_logic(
                target_fetch_date
            )

            if call_status == "SUCCESS_DATA" and nordpool_day_payload:
                _LOGGER.info(
                    f"Successfully fetched data for {target_fetch_date}. Data points: {len(nordpool_day_payload.get('raw', []))} items. Currency: {nordpool_day_payload.get('currency')}"
                )
                ...
                if new_currency:
                    if self._currency and self._currency != new_currency:
                        _LOGGER.debug(
                            f"Currency changed from {self._currency} to {new_currency}. Using new currency."
                        )
                    self._currency = new_currency
                elif not self._currency:
                    _LOGGER.debug(
                        f"Fetched data for {target_fetch_date} has no currency, using configured currency if available."
                    )
                ...
                if current_operation_type == "TODAY":
                    ...
                    _LOGGER.debug(
                        f"Stored TODAY data: {len(new_raw_data) if new_raw_data else 0} items for {target_fetch_date}"
                    )
                elif current_operation_type == "TOMORROW":
                    self._data_for_next_hass_date = new_raw_data
                    self._date_of_next_data = target_fetch_date
                    _LOGGER.debug(
                        f"Stored TOMORROW data: {len(new_raw_data) if new_raw_data else 0} items for {target_fetch_date}"
                    )
            elif call_status == "SUCCESS_NO_DATA":
                _LOGGER.warning(
                    f"Fetch for {target_fetch_date} returned SUCCESS but no data payload."
                )
            elif call_status != "NOT_ATTEMPTED":
                _LOGGER.warning(
                    f"Nordpool call for {target_fetch_date} (Op: {current_operation_type}) failed with status: {call_status}."
                )
        ...
        todays_data_is_now_ok = (
            self._data_for_current_hass_date
            and self._date_of_current_data == current_hass_date
        )
        expected_tomorrows_date = current_hass_date + timedelta(days=1)
        tomorrows_data_is_now_ok = (
            self._data_for_next_hass_date
            and self._date_of_next_data == expected_tomorrows_date
        )
        ...
        if not todays_data_is_now_ok:
            ...
            _LOGGER.warning(
                f"Today's data for {current_hass_date} is missing or stale ({self._date_of_current_data}). {new_log_state_name}"
            )
        elif not tomorrows_data_is_now_ok:
            # Today's data is OK, but tomorrow's is not.
            _LOGGER.info(
                f"Today's data for {current_hass_date} is OK. Next day's data ({expected_tomorrows_date}) is missing or stale ({self._date_of_next_data if self._date_of_next_data else 'None'})."
            )
            ...
            if (
                current_operation_type == "TODAY"
                and call_status == "SUCCESS_DATA"
                and todays_data_is_now_ok
                and hass_now >= time_13h00
            ):
                ...
                next_delay_seconds = max(
                    0.1, (next_run_time_target - hass_now).total_seconds()
                )
            ...
            _LOGGER.info(
                f"Data for today ({current_hass_date}) and tomorrow ({expected_tomorrows_date}) are up-to-date."
            )
            next_day_13h00 = (hass_now + timedelta(days=1)).replace(
        ...
        if self._task_remover[0]:
            try:
                ...
                _LOGGER.debug(
                    "Exception while trying to cancel previous Nordpool task.",
                    exc_info=True,
                )
        ...
        ):  # Ensure delay is positive, run almost immediately if calculated in past
            _LOGGER.warning(
                f"Calculated next_delay_seconds was {next_delay_seconds:.2f}. Adjusting to 0.1 second. State: {new_log_state_name}"
            )
            next_delay_seconds = 0.1
        elif (
            next_delay_seconds < 1 and next_delay_seconds != 0.1
        ):  # For very small positive delays (but not our special 0.1s), make it 1s
            _LOGGER.warning(
        ...
        _LOGGER.info(
            f"Scheduling next Nordpool call in {next_delay_seconds:.2f} seconds (New State: {new_log_state_name})."
        )
    ...
    def start(self) -> None:
        ...
        _LOGGER.info(
            f"Nordpool coordinator starting with currency='{self._currency}'. Scheduling initial data fetch. State: {self._current_schedule_state[0]}"
        )
        ...
        if self._task_remover[0]:
            try:
                ...
                _LOGGER.debug(
                    "Exception during pre-start cancel of existing task.", exc_info=True
                )
            self._task_remover[0] = None
    ...
    def stop(self) -> None:
        ...
        if self._task_remover[0]:
            _LOGGER.info(
                "Stopping Nordpool data coordinator and cancelling scheduled tasks."
            )
            try:
                self._task_remover[0]()
                self._task_remover[0] = None
                ...
                _LOGGER.warning(
                    f"Error while cancelling Nordpool task on stop: {e}", exc_info=True
                )
        else:
